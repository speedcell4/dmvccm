...
class DMVCCM(dmv.DMV):
    ...
    def p_head(self, k, i, j, s2, pio, end_seal, n2):
        ...
        r1 = (pi[i, j].val('<>' + hk) - pi[i, j].val('>' + hk) * self.p_stop_right(h, k == j - 1)) * po[i, j].val(
            '<>' + hk) / denom
        r2 = (pi[i, j].val('><' + hk) - pi[i, j].val('<' + hk) * self.p_stop_left(h, k == i)) * po[i, j].val(
            '><' + hk) / denom
    ...
    def p_inside_outside(self, s):
        ...
        p_outside = {
            (0, n): dmv.PIODict([
                (1.0, dmv.Node('|', self.end_symbol, n - 1)),
                (1.0, dmv.Node('<>', self.end_symbol, n - 1))
            ])
        }
        ...
        for l in range(n - 1, 0, -1):
            for i in range(n - l + 1):
                ...
                for k in range(i):
                    for (p1, n1) in p_inside[k, i].itervalues():
                        for (p2, n2) in p_outside[k, j].itervalues():
                            ...
                            elif n1.mark == '|' and n2.mark[0] == '<' and i <= n2.index < j:
                                # m-n1.index = distancia entre uno y otro
                                m = n2.index
                                h = n2.word
                                # DMVCCM: multiplicar por phi:
                ...
                p_outside[i, j] = dmv.PIODict(
                    sum((self.unary_p_outside(p, node, i, j, end) for (p, node) in p_outside_dict.itervalues()), []))
    ...
    def p_inside(self, s):
        ...
        for l in range(2, n + 1):
            for i in range(n - l + 1):
                ...
                p_inside[i, j] = dmv.PIODict(
                    sum((self.unary_p_inside(p, node, i, j) for (p, node) in p_inside_dict.itervalues()), []))
