...
class DMV(model.ProjDepModel):
    ...
    def MStep(self):
        ...
        for s in filter(lambda s: len(s) > 1, self.training_corpus.tagged_sents()):
            s = [x[1] for x in s]
            s = s + [self.end_symbol]
    ...
    def p_inside_outside(self, s):
        ...
        n = len(s)
        p_outside = {
            (0, n): PIODict([(1.0, Node('|', self.end_symbol, n - 1)), (1.0, Node('<>', self.end_symbol, n - 1))])
        }
